    ...
    protected Integer doInBackground(Void... params) {
        ...
        if (!isOnline()) {
            ...
            if (table == null) {
                ...
                if (prog == null) {
                    Log.e(TAG, "No data!");
                    AppMsg.makeText((MainActivity) mCtx, R.string.error_pl_nodata, AppMsg.STYLE_ALERT).show();
                    return 1;
                } else {
                    for (HashMap<String, Object> m : prog) {
                        ContentValues values = new ContentValues();
                        values.put(DBConstants.KEY_DATUM, ((Date) m.get(DBConstants.KEY_DATUM)).getTime());
                        values.put(DBConstants.KEY_THEMA, (String) m.get(DBConstants.KEY_THEMA));
                        values.put(DBConstants.KEY_PERSON, (String) m.get(DBConstants.KEY_PERSON));
                        values.put(DBConstants.KEY_EDIT, 0);
                        Log.d(TAG, "Datum: " + values.get(DBConstants.KEY_DATUM));
                        Uri uri = mCtx.getContentResolver().insert(DBConstants.TABLE_CONTENT_URI, values);
                        Log.d(TAG, uri.toString());
//                        mDbHelper.createEntry((Date) m.get(DbHelper.KEY_DATUM),
//                                (String) m.get(DbHelper.KEY_THEMA),
//                                (String) m.get(DbHelper.KEY_PERSON));
                    }
                }
            }
        }
